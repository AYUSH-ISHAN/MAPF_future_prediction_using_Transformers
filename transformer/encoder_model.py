...
class Encoder(nn.Module):
    """a encoder model0 with self attention mechanism"""
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        # except:
            # print(torch.get_device(x), torch.get_device(self.pos_table[:, :x.size(1)].clone().detach()))


class TransformerEncoder(nn.Module):
    """a sequence to sequence model0 with attention mechanism"""
    ...
    def forward(self, encoder_input, comm_mask):
        ...
        enc_output, enc_attn = self.encoder(encoder_input, comm_mask, True)
