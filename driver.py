        ...
        while curr_steps < TrainingParameters.N_MAX_STEPS:
            ...
            if demon:
                ...
                for results in range(done_len):
                    ...
                    mb_predicted_actions.append(job_results[results][6])
                    mb_curr_pose_time.append(job_results[results][7])
                ...
                mb_predicted_actions = np.concatenate(mb_predicted_actions, axis=0)
                mb_curr_pose_time = np.concatenate(mb_curr_pose_time, axis=0)
                ...
                for results in range(done_len):
                    ...
                    mb_coll_agents.append(job_results[results][14])
                    mb_predicted_actions.append(job_results[results][15])
                    mb_curr_pose_time.append(job_results[results][16])
                    curr_episodes += job_results[results][-2]
                ...
                mb_message = np.concatenate(mb_message, axis=0)
                mb_coll_agents = np.concatenate(mb_coll_agents, axis=0)
                mb_predicted_actions = np.concatenate(mb_predicted_actions, axis=0)
                mb_curr_pose_time = np.concatenate(mb_curr_pose_time, axis=0)
                    ...
                    for start in range(0, done_len * TrainingParameters.N_STEPS, TrainingParameters.MINIBATCH_SIZE):
                        ...
                        slices = (arr[mb_inds] for arr in
                                  (mb_obs, mb_vector, mb_returns_in, mb_returns_ex, mb_returns_all, mb_values_in,
                                   ...
                                   mb_train_valid, mb_blocking, mb_message, mb_coll_agents, mb_predicted_actions\
                                    ,mb_curr_pose_time))
                        mb_loss.append(global_model.train(*slices))

                if curr_steps % (COUNT*10**7) == 0:
                    title = "folder_mask/" + str(curr_steps) + "_masks"
                    np.save(title, mb_coll_agents)
                    COUNT+=0.5
...
def evaluate(eval_env, episodic_buffer, model, device, save_gif, curr_steps, greedy):
    ...
    for i in range(RecordingParameters.EVAL_EPISODES):
        ...
        done, valid_actions, obs, vector, _ ,coll_agent = reset_env(eval_env, num_agent)
        predicted_actions = np.zeros((1, EnvParameters.N_AGENTS, TrainingParameters.PREDICTION_TIMESTEPS_LIMIT), dtype=np.float32)
        curr_pose_time = np.zeros((1, EnvParameters.N_AGENTS, (TrainingParameters.PREDICTION_TIMESTEPS_LIMIT+1)*2), dtype=np.float32)
        ...
        while not done:
            ...
            actions, pre_block, hidden_state, num_invalid, v_all, ps, message = model.evaluate(obs, vector,
                                                                                               ...
                                                                                               message, num_agent, coll_agent, predicted_actions, curr_pose_time)
            one_episode_perf['invalid'] += num_invalid

            # move
            rewards, valid_actions, obs, vector, _, done, _, num_on_goals, one_episode_perf, max_on_goals, \
                _, _, on_goal, coll_agent, predicted_actions, curr_pose_time = one_step(eval_env, one_episode_perf, actions, pre_block, model, v_all, hidden_state,
